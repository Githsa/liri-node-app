  ...
  user.get('statuses/user_timeline', screenName, function(error, tweets, response){
    if(!error){
      for(var i = 0; i<tweets.length; i++){
        ...
        console.log("@SeanAlmers: " + tweets[i].text + " Created At: " + date.substring(0, 19));
        console.log("-----------------------");
        
        
        fs.appendFile('log.txt', "@SeanAlmers: " + tweets[i].text + " Created At: " + date.substring(0, 19));
...
function omdbData(movie){
  ...
  request(omdbURL, function (error, response, body){
    if(!error && response.statusCode == 200){
      ...
      console.log("Release on: " + body.Year);
